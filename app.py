...
def show_user_profile():
    ...
    if not st.session_state.edit_mode:
        ...
        avatar_options = {"🦁 Lion": "https://api.dicebear.com/7.x/adventurer/svg?seed=Lion",
                          "🐯 Tiger": "https://api.dicebear.com/7.x/adventurer/svg?seed=Tiger",
                          "🐉 Dragon": "https://api.dicebear.com/7.x/adventurer/svg?seed=Dragon"}  # Add more if needed
...
if menu == "Login":
    ...
    avatar_options = {"🦁 Lion": "https://api.dicebear.com/7.x/adventurer/svg?seed=Lion"}  # Simplified
    avatar_name = st.selectbox("Avatar", list(avatar_options.keys()))
    avatar_url = avatar_options[avatar_name]
    st.image(avatar_url, width=100)
    ...
    locations = ["🏠 My Home", "🏫 My School", "✍️ Other"]
    choice = st.selectbox("Choose a place", locations)
    location = st.text_input("Describe your place") if choice == "✍️ Other" else choice
    ...
    if st.button("Generate Palace") and topic and location:
        try:
            ...
            db.collection("users").document(st.session_state["user"]["localId"]).collection("palaces").add({
                "topic": topic, "scene": scene, "translated_scene": translated_scene,
                "location": location, "created_at": datetime.utcnow()
            })
            st.success("📂 Palace saved!")
        except Exception as e:
            st.error("❌ Error generating scene")
